                ...
                <div className='flex flex-col my-'>
                    ...
                    <div className='flex justify-between items-center bg-[#F4E7FB] dark:bg-slate-800 border-[#E5E5E5] dark:border-slate-900 p-3 border-2 rounded-b-lg'>
                        <div className='flex justify-between items-center'>
                            <img src={data.user.profile_image.small} className='rounded-full md:w-10 w-5 mr-3' alt="Profile Image" />
                            <div>
                                <p className='md:text-sm text-[0.389rem] font-bold'> {data.user.first_name} <span>{data.user.last_name} </span> </p>
                                <p className='text-[#A7A7A7] md:text-xs text-[0.342rem] font-semibold '>@{data.user.instagram_username}</p>
                            </div>
                        </div>
                        <div className='flex justify-between items-center'>
                            <img src={likes} alt="" className='md:w-5 w-2 mr-1' />
                            <p className='md:text-xs text-[0.342rem] font-bold'>{data.user.total_likes}</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <Fade in={open}>
                    <Box sx={style} className="w-4/5 md:w-2/5 rounded-xl">
                        <div className='relative'>
                            <img src={data.urls.small} alt={data.alt_description} className='relative rounded-t-lg w-full md:max-h-[500px] max-h-[800px] ' />
